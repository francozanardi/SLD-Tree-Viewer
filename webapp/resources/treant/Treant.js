...
;( function() {
    ...
    Tree.prototype = {
        ...
        addNode: function( parentTreeNode, nodeDefinition ) {
            ...
            // oNewNode.parent().createSwitchGeometry( this ); //comentamos esto para que no se agregue el botón de collapse.
		 ...
		 * @returns {array}
         */
		getCutLines: function(nodo){
			const DIST_RECTAS = 4;
			const TAM_RECTAS = 10;
			
			var p1 = nodo.parent().connectorPoint(true);
			var p2 = nodo.connectorPoint(false);
			...
			var vectPerpendicular = {
				x: 1,
				y: -(vect.x/vect.y)
			};
			
			var modVectP = Math.hypot(vectPerpendicular.x, vectPerpendicular.y);
			vectPerpendicular.x = (vectPerpendicular.x/modVectP)*(TAM_RECTAS/2);
			vectPerpendicular.y = (vectPerpendicular.y/modVectP)*(TAM_RECTAS/2);
			
			// var slope = (vect.y/vect.x)*10;
			
			var modVect = Math.hypot(vect.x, vect.y);
			
			var vGap = {}
			vGap.x = (vect.x/modVect)*(DIST_RECTAS/2);
			vGap.y = (vect.y/modVect)*(DIST_RECTAS/2);
			...
			var a = {
					x: puntoMedio.x + vGap.x + vectPerpendicular.x,
					y: puntoMedio.y + vGap.y + vectPerpendicular.y,
			};
			...
			var line1 = ["M", a.x + "," + a.y, 'L', b.x + "," + b.y];
			var line2 = ["M", c.x + "," + c.y, 'L', d.x + "," + d.y];
        ...
        setConnectionToParent: function( treeNode, hidePoint ) {
            ...
            if ( this.connectionStore[treeNode.id] ) {
				...
				if(treeNode.hayCut){
					connLine = this.connectionStore[treeNode.id].connector;
					this.animatePath( connLine, pathString );
					
					this.animatePath( this.connectionStore[treeNode.id].cut, this.getCutLines(treeNode).join(' '));
    ...
    TreeNode.prototype = {
         ...
         */
        setTextName: function(newName) {
			this.text.name = newName;
			this.nodeDOM.querySelector('.node-name').innerHTML = newName;
			this.width = this.nodeDOM.offsetWidth;
			
			this.getTree().redraw();			
        },
		
        /**
		 * Realiza un cut a la rama padre del nodo.
         */
        cutRama: function() {
			var cutLines = this.getTree().getCutLines(this);
			var cut = this.getTree()._R.path(cutLines.join(" "));
			
			this.hayCut = true;
			
			this.getTree().connectionStore[this.id] = {
